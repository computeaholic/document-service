...
@pytest.fixture
def postgres_engine(postgres_url: str):  # type: ignore[no-untyped-def]
...
@pytest.fixture
def postgres_session_factory(postgres_engine):  # type: ignore[no-untyped-def]
    """Create session factory for tests."""
    return sessionmaker(bind=postgres_engine)


def test_postgres_persists_created_and_updated_timestamps(
    postgres_session_factory: sessionmaker,  # type: ignore[type-arg]
) -> None:
